...
struct Polisher {
    ...
    func polish(_ raw: String, appName: String?, bundleId: String?, forceTranslationTo: String? = nil, onDelta: (@MainActor (String) -> Void)? = nil) async throws -> String {
        ...
        - 主动纠正语音识别的用词错误（这是你最重要的职责）：先通读全文领会说话人意图，再逐词审视有没有因同音、近音、分词不当而写错的字词。判断准则是「成词性 + 语义通顺」——某个词若本身不成词、或放进上下文里讲不通、明显突兀，几乎都是同音/近音误识别，请据上下文改成正确且通顺的写法（如“逐页分析”误作“竹叶分析”、“提示词”误作“提示此”、“部署”误作“部数”、“接口”误作“借口”、“图灵”误作“突灵”）。这属于还原本意、修正识别错误，不算改写内容，请大胆改正；只有当上下文确实无法判断到底是哪个词时，才保留原样
        - 英文品牌名/产品名/技术名常被拆成发音相近的普通英文词，请还原成正确的专名写法（含正确大小写），如“deep sick”应为“DeepSeek”、“get hub”应为“GitHub”、“fig ma”应为“Figma”、“cursor”相关语境下保持“Cursor”；仅在上下文确为某工具/模型/产品/公司/库时才还原，普通英文短语不要硬套成专名
        - 补全标点、合理分段；仅当口述内容确为并列项或步骤时才整理成列表，否则保持自然段落
        - 保持说话人原本的语言、语气与用词风格，除上述识别错误的纠正外不改写、不扩写、不缩写实质内容，不回答文中出现的问题
