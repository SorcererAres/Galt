...
# 固定签名身份（自签名证书），保证重新编译后辅助功能等授权不失效；缺失时回退 ad-hoc
SIGN_ID := $(shell security find-identity -v -p codesigning 2>/dev/null | grep -q "Galt Dev Signing" && echo "Galt Dev Signing" || echo "-")
...
app: build
	...
	codesign --force --sign "$(SIGN_ID)" $(DIST)/Contents/Frameworks/whisper.framework
